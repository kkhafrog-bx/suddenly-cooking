export default async function handler(req, res) {
  ...
  try {
          ...
          body: JSON.stringify({
            contents: [{ 
              parts: [{ 
                ...
                2. '이 부분', '요리명', '재료 리스트' 같은 메타 텍스트나 설명용 괄호를 쓰지 마세요.
                3. 아래의 형식을 엄격히 지키세요:
                ...
                4. 각 단계 앞에는 내용과 어울리는 이모지를 하나씩 붙이세요.` 
              }]
            }]
          })
        });
    ...
    if (recipe) {
      // 안전장치: 별표와 불필요한 지시문 텍스트 강제 제거
      const cleanRecipe = recipe.replace(/\*\*/g, '').replace(/\(이 부분\)/g, '');
